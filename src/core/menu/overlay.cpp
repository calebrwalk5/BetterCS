...
void Menu::drawOverlay(ImDrawList* drawList) {
    ...
    if(!CONFIGBOOL("Misc>Misc>Misc>Disable Watermark")) {
        ...
        sprintf(watermarkText, "BetterCS (%s - %s@%s) | %.1f FPS | %i ms", distro, getpwuid(getuid())->pw_name, hostname, ImGui::GetIO().Framerate, (Interfaces::engine->IsInGame() && playerResource) ? playerResource->GetPing(Interfaces::engine->GetLocalPlayer()) : 0);
