    ...
    for line in code.split('\n'):
        if 'ImVec4(' in line:
            ...
            if len(values) >= 4:
                ...
                if g > r and g > b:
                    print(f"Green value found: {g:.2f}")
...
void Menu::drawOverlay(ImDrawList* drawList) {
    ...
    if(!CONFIGBOOL("Misc>Misc>Misc>Disable Watermark")) {
        ...
        sprintf(watermarkText, "N3WTuX (%s - %s@%s) | %.1f FPS | %i ms", distro, getpwuid(getuid())->pw_name, hostname, ImGui::GetIO().Framerate, (Interfaces::engine->IsInGame() && playerResource) ? playerResource->GetPing(Interfaces::engine->GetLocalPlayer()) : 0);
